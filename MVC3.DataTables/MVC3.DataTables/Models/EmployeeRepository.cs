...
namespace MVC3.DataTables.Models
{
	public class EmployeeRepository
	{
		...
		public IList<Employee> GetEmployees(string sort, string direction, int page, int itemsPerPage)
		{
			int startIndex = (page - 1) * itemsPerPage;

			var list = (from e in _employeeList
						select e).OrderBy(x => x.FirstName).Skip(startIndex).Take(itemsPerPage);
